...
import ema.dve.workload.csv.CSVLayerIncremental;
...
public class WorkloadGeneratorIncremental
{
	...
	private final AStaticPlacement hotspotPlacement;
	private final AStaticPlacement objectPlacement;
	private IAvatarNumberFunction avatarNumber;
	private final IdProvider avatarID;

	public WorkloadGeneratorIncremental(final PropertiesPersonalized conf, final AMobilityModel mobility)
	{
		this.conf = conf;
		...
		if(conf.getObjectPlacement().equals("random"))
		{
			this.objectPlacement = new RandomObjectPlacement(conf, new IdProvider(2));
		} else
		{
			this.objectPlacement = new HotspotProbabilityObjectPlacement(conf, new IdProvider(2));
		}
		...
		this.csv = new CSVLayerIncremental(conf, map.getDimension());
//		this.csv.setFileName(conf.getProperty("OUTPUT_FILE"));
	...
	public OracleIncremental process() throws Exception
	{
		...
		// init the oracle
		final OracleIncremental oracle = new OracleIncremental(conf);
		...
		if (this.dump)
		{
			csv.writeStatic(oracle);
		}

		int numberInitialTransfer = 0;
		...
		for (int i=1; i<iterations; i++)
		{
			...
			numberInitialTransfer += oracle.getNumberInitialTransfer();

			oracle.setAOIStatAt(GlobalTime.getCurrent(), computer.computeAOI(map, ex));

			// update the oracle
			// Map copy = new Map(map);
			// not required deep copy, i write immediately to file
			oracle.setMapAt(GlobalTime.getCurrent(), map);
		...
		stats.write(mobility.getName()+","+
					avatarNumber.getMaxNumber()+","+
					(end-start)+","+
					conf.getPropertyInt("THREAD_NUMBER", 4)+","+
					conf.getObjectPlacement()+","+
					numberInitialTransfer+","+
					conf.getPropertyInt("OBJECTS_NUM")+","+
					"\n");
		stats.flush();
		stats.close();
