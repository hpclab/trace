 ...
 * @throws IOException
 */
private static void findClasses(final Map<String,String> map)
throws IOException
{
	final String classPath = System.getProperty( "java.class.path" );
	final String separator = System.getProperty( "path.separator"  );
	final String filesep = System.getProperty( "file.separator");
	final StringTokenizer path = new StringTokenizer( classPath, separator );
 ...
 * @throws IOException
 */
private static void findClassInJar(final Map<String,String> map, final File pathFile)
throws IOException
{
	final ZipFile zipFile = new ZipFile( pathFile );
	final Enumeration entries = zipFile.entries();
	while( entries.hasMoreElements() ) {

		final String entry = entries.nextElement().toString();
		if( entry.endsWith( ".class" ) ) {
			// File names in ZIP archives (so, also in JARs)
			// are separated by forward slashes '/', independently
			// of the architecture.
			final String className = classname( entry, "/" );
 ...
 * @throws IOException
 */
private static void findClassInPathDir( final Map<String,String> map,
	final String pathElement, final File pathFile )
throws IOException
{
	visitedDirs++;
	if(visitedDirs>=maxDirs)
	...
	final String[] list = pathFile.list();
	final String filesep = System.getProperty( "file.separator");

	for( int i = 0; i < list.length; i++ ) {
		final File file = new File( pathFile, list[i] );
		if( file.isDirectory() ) {
			findClassInPathDir( map, pathElement, file );
		}
		else if ( file.exists() && (file.length() != 0) && list[i].endsWith( ".class" ) ) {
			final String classFile = file.toString().substring( pathElement.length());
			final String className = classname( classFile, filesep );
			final String shortName = getShortName( className );
			if (map.containsKey(shortName)) {
 ...
 */
private static String classname(final String classFile, final String filesep)
{
