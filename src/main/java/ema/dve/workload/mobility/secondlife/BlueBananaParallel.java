 ...
 *
 */
public class BlueBananaParallel extends AMobilityModel
{
	private Map<Avatar, Direction> directions;
	private Map<Avatar, Automaton> automata;
	...
	public BlueBananaParallel(final PropertiesPersonalized configuration)
	{
		super(configuration);
		...
		directions = new ConcurrentHashMap<Avatar, Direction>();
		automata = new ConcurrentHashMap<Avatar, Automaton>();
	}
	...
	public void move(final int time, final MapVirtualEnvironment map, final ExecutorService ex_)
	{
		...
		final ConcurrentHashMap<Avatar, Direction> new_directions = new ConcurrentHashMap<Avatar, Direction>();
		final ConcurrentHashMap<Avatar, Automaton> new_automata = new ConcurrentHashMap<Avatar, Automaton>();
			...
			w.add(ex_.submit(new Callable<Integer>()
			{
				...
				public Integer call()
				{
					...
					else
					{
						...
						direction = computeDirection(map, a, direction, previous, current, time);
